...
public class OpenMenuAction implements Action {
    ...
    public OpenMenuAction(final String serializedAction) { this.targetMenu = PlaceholderString.of(serializedAction); }

    @Override
    public void execute(final Player player) {
        final String menuName = this.targetMenu.getValue(player);
        final InternalMenu menu = MenuManager.getMenuByFileName(menuName);

        if (menu != null) {
            /*
             * Delay the task, since this action is executed in ClickInventoryEvent and
             * opening another inventory in the same moment is not a good idea.
             */
