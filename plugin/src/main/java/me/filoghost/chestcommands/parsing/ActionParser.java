...
public class ActionParser {
    ...
    public static Action parse(String serializedAction) throws ParseException {
        for (final ActionType actionType : ActionType.values()) {
            final Matcher matcher = actionType.prefixPattern.matcher(serializedAction);
            if (matcher.find()) {
    ...
    private enum ActionType {
        ...
        CONSOLE_COMMAND("console", ConsoleCommandAction::new), OP_COMMAND("op", OpCommandAction::new), OPEN("open", OpenMenuAction::new),
        SERVER("server", ChangeServerAction::new), // The colon is optional
        TELL("tell", SendMessageAction::new), BROADCAST("broadcast", BroadcastAction::new), GIVE_ITEM("give", GiveItemAction::new),
        GIVE_MONEY("give-money", GiveMoneyAction::new), SOUND("sound", PlaySoundAction::new), BOSS_BAR("dragon-bar", DragonBarAction::new);
